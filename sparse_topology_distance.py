...
parser.add_argument('--layer', default=0, type=int, help='Topology graph of which layer to measure')
...
parser.add_argument('--save', metavar='DIR', default='save/', help='path to save the topology distance matrices')
...
def save_obj(obj, name):
    with open(args.save + name + '.pkl', 'wb') as f:
        pickle.dump(obj, f, pickle.HIGHEST_PROTOCOL)

def load_obj(name):
    try:
        with open(args.save + name + '.pkl', 'rb') as f:
            dic = pickle.load(f)
